...
def TextConstructor(string="Hello World"):
    # TODO: Add Spaces
...
def LineLocator(img):
    '''
    --------------------------------------
    Locates the Lines in the Image
    --------------------------------------
    @Params: img: image to be Cropped
    ...
    coords = CoordinateData(dictionaryMode=True) #getting the coordinates
    LineHeight = 19.5
    LineHeightTop = coords["tabs-lower"]
    LineHeightBottom =LineHeightTop + LineHeight
    LineLocation = 0
    while LineLocation < 500:
        LineHeightTop = LineHeightBottom
        LineHeightBottom = LineHeightTop + 20
        LineLocation = LineLocation + 20
        Line = Crop(img,showing = False , up = LineHeightTop, down = LineHeightBottom  , left = coords["window-left"], right = coords["window-right"])
        show(Line)

def GetLineCoordinates(img):
    '''
    --------------------------------------
    Gets the Coordinates of the Lines in the Image
    --------------------------------------
    @Params: img: image to be Cropped
    ...
    while LineLocation < 500:
        ...
        LineHeightBottom = LineHeightTop + 20
        LineLocation = LineLocation + 20
        print(LineHeightTop,LineHeightBottom)
...
def LineBuilder(img,showing = False,line = 1,string = 'hi'):
    '''
    --------------------------------------
    Builds the Text
    --------------------------------------
    ...
    img[5+LineOffset:20+LineOffset ,0:CharacterLenght ] = text[5:20 ,0:CharacterLenght]
    if showing:
        show(img)
    return img
if __name__ == '__main__':
    ...
    img = cv2.imread(path)
    img = Window(img)
    string = ' Ayo what up im taking over this joint'
    

    LineBuilder(img,showing = True,line = 2,string = string)
    
    
    # text = TextConstructor(string)
    img
    # img[10:30,0:36] = text[0:20,0:36]
    # img[21:40,0:36] = text[1:20,0:36]
    # show(img)
