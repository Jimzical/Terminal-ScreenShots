...
def LineBuilder(img,showing = False,line = 1,string = 'hi',overwrite = False):
    ...
    img[Yoffset+LineOffset:20+LineOffset ,0:CharacterLenght ] = text[Yoffset:20 ,0:CharacterLenght]
    if showing:
        show(img)
    return img
...
def TODO():
    todo = '''  Fix setup, get brand new alphabeths
                Create GUI
                Create a GitHub Pages
                Clean the Entire system

                Multiple Lines Creation
                Possiblly use image detection for automation of window selection
                Change Bedsheets
           '''
    print(todo)
